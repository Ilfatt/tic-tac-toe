...
const Container = styled.div<{width?: number | string; height?: number}>`
  ...
  ${(props) => (props.height ? `height: ${props.height}px` : 'max-height: 85vh;')};
`;
...
const Body = styled.div<{ footer: Props['footer']; clearPaddings?: boolean; noScroll?: boolean }>`
  ${(props) => (props.noScroll
    ? ''
    : 'overflow: auto;'
  )}
  padding: ${(props) => (props.footer ? '0 46px 40px 46px' : '0 46px 46px 46px')};
